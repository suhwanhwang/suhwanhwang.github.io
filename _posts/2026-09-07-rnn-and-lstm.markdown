...
```text
h_new = tanh(W_h × h_old + W_x × x + b)
```

- `h_old`: 이전까지의 요약 (512차원)
- `x`: 이번 단어의 임베딩 (512차원)
- `W_h`, `W_x`: 학습되는 가중치
- `tanh`: 활성화 함수. 출력을 -1~1로 눌러줌

앞선 글에서 본 신경망 층 그대로다. 입력이 두 개일 뿐 새로운 게 없다.
논문 1장의 표현으로는, 은닉 상태 h_t를 이전 은닉 상태 h_{t−1}과 위치 t의 입력의 함수로 생성한다.
...
**병목이 보인다.** 원문 전체가 `h₃`라는 512차원 벡터 하나에 담겨야 한다.
문장이 길어지면 담을 수 없다.

이 병목을 완화하려고 나온 게 **원래의 어텐션**이다.
디코더가 `h₃` 하나만 보는 대신 `h₁`, `h₂`, `h₃`를 전부 보되 필요한 것에 가중치를 주게 한 것이다.
...
```python
h₁ = f(h₀, "앱이")      # 먼저
h₂ = f(h₁, "느려서")    # h₁이 있어야 가능
h₃ = f(h₂, "지웠다")    # h₂가 있어야 가능
```
...
```text
스케일 0.9 → 0.9^50 = 0.005   (0.5%만 도달)
스케일 0.8 → 0.8^50 ≈ 0.00001 (사실상 0)
스케일 1.1 → 1.1^50 ≈ 117     (기울기 폭발, 발산)
```
...
**핵심은 곱셈 대신 덧셈으로 기억을 나른다는 것이다.**

`h`와 별도로 **셀 상태(cell state)** 통로를 하나 더 둔다.
이 통로는 갱신될 때 곱셈이 아니라 덧셈으로 정보를 얹는다.

```text
RNN:  h = tanh(W × h + ...)              ← 매번 W가 곱해짐
LSTM: c = (망각게이트 × c) + (새 정보)    ← 덧셈이 주된 경로
```

어디서 본 구조다. 잔차 연결 `x + Sublayer(x)`와 같은 원리다.
서로 다른 맥락에서 나온 해법인데 아이디어가 같다.

게이트는 세 개다. 각각 0~1을 내놓는 작은 신경망이고, 전부 학습된다.

| 게이트 | 하는 일 |
|---|---|
| 망각 게이트 | 기존 기억 중 뭘 버릴지 (0이면 삭제, 1이면 유지) |
| 입력 게이트 | 새 정보 중 뭘 받아들일지 |
| 출력 게이트 | 기억 중 뭘 내보낼지 |

덕분에 "이 정보는 중요하니 30단어 뒤까지 들고 간다"가 가능해진다.

**GRU**는 게이트를 두 개로 줄인 단순화 버전이다.
...
| 순환(RNN/LSTM) | O(n·d²) | O(n) | O(n) |
| 합성곱 | O(k·n·d²) | O(1) | O(log_k n) |
...
**대신 치른 대가**가 층당 복잡도 O(n²·d)다.
저자들은 문장 길이 n이 표현 차원 d보다 작으면 오히려 셀프 어텐션이 더 빠르고,
당시 번역 모델에서는 대개 그랬다고 논증한다.

> n=50, d=512면 셀프 어텐션이 유리하다.
> 하지만 n이 10만 토큰이 되면 이야기가 달라진다.
